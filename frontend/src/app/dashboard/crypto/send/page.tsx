...
function CoinIcon({ symbol }: { symbol: string }) {
  ...
  if (errored) {
    return (
      <div className="w-12 h-12 rounded-full bg-gray-200 dark:bg-gray-700 flex items-center justify-center text-xs font-semibold text-gray-500 dark:text-gray-400 shrink-0">
        {symbol.slice(0, 2)}
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col min-h-full bg-white dark:bg-[#050505]">
      ...
      <div className="px-4 py-4 shrink-0">
        <div className="flex items-center gap-2.5 bg-gray-100 dark:bg-[#101010] rounded-xl px-4 h-11">
        ...
        <div className="flex-1 overflow-y-auto">
          {filtered.map((asset, i) => (
            <div key={asset.id}>
              ...
              {i < filtered.length - 1 && (
                <div className="h-px bg-gray-100 dark:bg-[#101010] mx-5" />
              )}
            </div>
          ))}
        </div>
