...
async function sendSignupOTPEmail(to, firstName, otp) {
  ...
  const html = `
    <div style="font-family:sans-serif;max-width:480px;margin:0 auto;background:#0d0b1e;color:#f5f3ff;padding:40px;border-radius:16px;">
      <h2 style="color:#8b5cf6;margin-bottom:4px;">Digital Wealth Solution</h2>
      <p style="color:#a78bfa;margin-bottom:28px;margin-top:0;">Email Verification</p>

      <p>Hi <strong>${firstName}</strong>,</p>
      <p>To complete your account registration, enter the verification code below on the sign-up page:</p>

      <div style="background:#1a1838;border:1px solid #7c3aed;padding:24px;text-align:center;border-radius:12px;margin:24px 0;">
        <span style="font-size:44px;font-weight:bold;letter-spacing:14px;color:#a78bfa;">${otp}</span>
      </div>

      <p style="color:#6b7280;font-size:13px;">
        This code expires in <strong style="color:#f5f3ff;">10 minutes</strong>.
        If you did not attempt to register an account, please ignore this email.
      </p>
      <hr style="border-color:#1a1838;margin:24px 0;" />
      <p style="color:#6b7280;font-size:12px;">© Digital Wealth Solution — do not reply to this email.</p>
    </div>
  `;
  ...
  await transporter.sendMail({
    from: FROM(),
    to,
    subject: 'Verify your email — Digital Wealth Solution',
    html,
  });
}
...
async function sendDepositNotificationEmail({ adminEmail, user, asset, amount, usdValue, txHash, depositId }) {
  ...
  const usdLine = usdValue
    ? `<tr><td style="color:#9ca3af;padding:6px 0;">USD Value</td><td style="color:#f5f3ff;font-weight:600;text-align:right;">≈ $${parseFloat(usdValue).toLocaleString('en-US', { minimumFractionDigits: 2 })}</td></tr>`
    : '';

  const txHashLine = txHash
    ? `<tr><td style="color:#9ca3af;padding:6px 0;">Transaction Hash</td><td style="color:#a78bfa;font-weight:600;text-align:right;word-break:break-all;">${txHash}</td></tr>`
    : '';

  const now = new Date().toLocaleString('en-US', { timeZone: 'UTC', dateStyle: 'medium', timeStyle: 'short' });

  const html = `
    <div style="font-family:sans-serif;max-width:560px;margin:0 auto;background:#0d0b1e;color:#f5f3ff;padding:40px;border-radius:16px;">
      <h2 style="color:#8b5cf6;margin-bottom:4px;">Digital Wealth Solution</h2>
      <p style="color:#a78bfa;margin-bottom:28px;margin-top:0;">New Deposit Submitted — Action Required</p>
      ...
      <!-- User details -->
      <h3 style="color:#a78bfa;font-size:13px;text-transform:uppercase;letter-spacing:1px;margin-bottom:10px;">User</h3>
      <table style="width:100%;border-collapse:collapse;margin-bottom:24px;">
        <tr><td style="color:#9ca3af;padding:6px 0;">Name</td><td style="color:#f5f3ff;font-weight:600;text-align:right;">${user.firstName} ${user.lastName}</td></tr>
        <tr><td style="color:#9ca3af;padding:6px 0;">Email</td><td style="color:#f5f3ff;text-align:right;">${user.email}</td></tr>
        <tr><td style="color:#9ca3af;padding:6px 0;">User ID</td><td style="color:#6b7280;font-size:12px;text-align:right;">${user.id}</td></tr>
      </table>

      <!-- Deposit details -->
      <h3 style="color:#a78bfa;font-size:13px;text-transform:uppercase;letter-spacing:1px;margin-bottom:10px;">Deposit Details</h3>
      <table style="width:100%;border-collapse:collapse;margin-bottom:24px;">
        <tr><td style="color:#9ca3af;padding:6px 0;">Asset</td><td style="color:#f5f3ff;font-weight:600;text-align:right;">${asset.name} (${asset.symbol})</td></tr>
        <tr><td style="color:#9ca3af;padding:6px 0;">Amount</td><td style="color:#f5f3ff;font-weight:600;text-align:right;">${amount} ${asset.symbol}</td></tr>
        ${usdLine}
        ${txHashLine}
        ...
        <tr><td style="color:#9ca3af;padding:6px 0;">Submitted At</td><td style="color:#f5f3ff;text-align:right;">${now} UTC</td></tr>
      </table>

      <!-- Wallet -->
      <h3 style="color:#a78bfa;font-size:13px;text-transform:uppercase;letter-spacing:1px;margin-bottom:10px;">Your Wallet Address (${asset.symbol})</h3>
      <div style="background:#1a1838;border:1px solid #7c3aed;padding:14px 18px;border-radius:10px;margin-bottom:28px;word-break:break-all;">
        <span style="color:#a78bfa;font-weight:600;font-size:14px;">${asset.walletAddress}</span>
      </div>

      <a href="${process.env.FRONTEND_URL || 'http://localhost:3000'}/admin/deposits"
      ...
      <hr style="border-color:#1a1838;margin:28px 0;" />
